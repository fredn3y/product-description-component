...
const _e = class _e extends HTMLElement {
  ...
  render() {
    ...
    try {
      ...
      e2.type = "application/ld+json", e2.textContent = this.generateSchema(), document.head.appendChild(e2);
      let i = "";
      if ("feature-heavy" === this._theme) console.log("Using feature-heavy template"), i = `
          <article class="product-container" itemscope itemtype="https://schema.org/Product">
            ...
            <section class="product-highlights">
              ${Object.entries(n2.productHighlights || {}).map(([n3, e3]) => `
                <div class="highlight-block">
                  <div class="highlight-content">
                    <h3 class="highlight-title">${e3.title}</h3>
                    <p class="highlight-description">${e3.description}</p>
                  </div>
                  ${e3.image ? `
                    <div class="highlight-image">
                      <img src="${e3.image}" alt="${e3.title}" loading="lazy">
                    </div>
                  ` : ""}
                </div>
              `).join("")}
            ...
            <section class="specifications-section">
              ...
              <div class="specs-table">
                ${Object.entries(n2.specifications ?? {}).map(([n3, e3]) => `
                  <div class="specs-row">
                    <div class="specs-label">${n3.charAt(0).toUpperCase() + n3.slice(1)}</div>
                    <div class="specs-value">${e3}</div>
                  </div>
                `).join("")}
              </div>
            </section>
        ...
        console.log("Using simple template"), console.log("Specifications:", n2.specifications), console.log("Contents:", n2.contents);
        const e3 = Object.entries(n2.specifications ?? {}).map(([n3, e4]) => `
          <div class="specs-row">
            <div class="specs-label">${n3.charAt(0).toUpperCase() + n3.slice(1)}</div>
            <div class="specs-value">${e4}</div>
          </div>
        `).join("");
        console.log("Generated spec rows:", e3);
        const t2 = (((_b = n2.contents) == null ? void 0 : _b.colors) ?? []).map((n3) => `
          <li class="contents-item">${n3}</li>
        `).join("");
        console.log("Generated content items:", t2), i = `
          <article class="product-container" itemscope itemtype="https://schema.org/Product">
            ${this._error ? `<div class="error-message">${this._error}</div>` : ""}
      ...
      const e2 = n2 instanceof Error ? n2.message : "Unknown error";
      this._error = `Error rendering product description: ${e2}`, console.error("Render error:", this._error), this._shadow.innerHTML = `
        <style>${t}</style>
